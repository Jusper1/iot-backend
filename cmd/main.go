...
func main() {
	...
	inaprocRepository := repositories.NewInaprocRepository(db)
	manualRepository := repositories.NewManualRepository(db)
	spjRepository := repositories.NewSPJRepository(db)
	pembelianAlatRepository := repositories.NewPembelianAlatRepository(db)
	dashboardRepository := repositories.NewDashboardRepository(db)

	// SERVICE
	authService := services.NewAuthService(adminRepository,passwordResetRepository)
	inaprocService := services.NewInaprocService(inaprocRepository)
	manualService := services.NewManualService(manualRepository)
	spjService := services.NewSPJService(spjRepository)
	pembelianAlatService := services.NewPembelianAlatService(pembelianAlatRepository)
	dashboardService := services.NewDashboardService(dashboardRepository)

	// HANDLER
	authHandler := handlers.NewAuthHandler(authService)
	inaprocHandler := handlers.NewInaprocHandler(inaprocService)
	...
	spjHandler := handlers.NewSPJHandler(spjService)	
	spjExcelHandler := handlers.NewSPJExcelHandler(spjService)
	pembelianAlatHandler := handlers.NewPembelianAlatHandler(pembelianAlatService)
	pembelianAlatExcelHandler := handlers.NewPembelianAlatExcelHandler(pembelianAlatService)
